class Pixel {
  /**
   * Create a pixel with defined colors
   * @param {'red' | 'green' | 'yellow' | 'hover'} color
   */
  constructor(color) {
    this.color = color;
    this.COLOR_SET = {
      red: [255, 69, 0, 255],
      orange: [255, 168, 0, 255],
      yellow: [255, 214, 53, 255],
      darkGreen: [0, 163, 104, 255],
      lightGreen: [126, 237, 86, 255],
      darkBlue: [36, 80, 164, 255],
      blue: [54, 144, 234, 255],
      lightBlue: [81, 233, 244, 255],
      darkPurple: [129, 30, 159, 255],
      purple: [180, 74, 192, 255],
      fightTheSunrise: [255, 153, 170, 255],
      brown: [156, 105, 38, 255],
      black: [0, 0, 0, 255],
      darkGray: [137, 141, 144, 255],
      lightGray: [212, 215, 217, 255],
      white: [255, 255, 255, 255],
    };
  }

  getColorEntity() {
    return this.COLOR_SET?.[this.color]
      ? new Uint8ClampedArray(this.COLOR_SET?.[this.color])
      : new Uint8ClampedArray(Object.values(this.COLOR_SET)[0]);
  }

  getAvailableColors() {
    return this.COLOR_SET;
  }
}
